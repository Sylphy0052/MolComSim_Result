...
"""
    ...
    def write_detail_body(self, file_name):
        ...
        text += self.create_fig_text(all_data.get_mean_fig_name(), 10)
        text += self.create_fig_text(all_data.get_prob_cumprob_fig_name(), 430)
    ...
    def create_tab1(self, config):
        tab1 = ""
        for k in config.config_header:
            if k == "outputFile":
                continue
            tab1 += self.check_config(config.config_dict, k)
        return TAB1.format(tab1)

    def create_tab2(self, output_data):
        output_data_arr = output_data.to_array()
        is_ptime = output_data.is_ptime
        tab2 = ""
        for i in range(len(TAB2_HEADER)):
            if i == 8 and not is_ptime:
                continue
            tab2 += TAB2_1.format(TAB2_HEADER[i], round(output_data_arr[i], 3))

        tab2 += TAB2_1.format("Analytical Mean", round(output_data.analytical_model.rtt, 3))
    ...
    def create_tab3(self, analytical_model):
        ...
        model_arr = analytical_model.to_array()
        tab = ""
    ...
    def create_tab4(self, plot_data):
        ...
        for i in range(len(plot_data.plot_range)):
            tab4 += TAB4_1.format(plot_data.plot_range[i][0], plot_data.plot_range[i][1], len(plot_data.plt_data[i]), round(plot_data.prob[i], 1), round(plot_data.cum_prob[i], 1))
