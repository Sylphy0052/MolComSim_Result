    ...
    def parseCollisionFile(self, data):
        ...
        with open(file_name, 'r') as f:
            for line in f:
                ...
                self.collisions = all_collision.split('/')
                self.each_collision = collision_num.split('/')
    ...
    def parseRetransmissionFile(self, data):
        ...
        with open(file_name, 'r') as f:
            for line in f:
                if ',' in line:
                    datas = line.split(',')
                    retransmit_data = datas[0].split('/')
                    self.retransmit_steps.extend(retransmit_data)
                    self.decomposing_num.append(datas[1])
                else:
                    retransmit_data = line.split('/')
                    self.retransmit_steps.extend(retransmit_data)
