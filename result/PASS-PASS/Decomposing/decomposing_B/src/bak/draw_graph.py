...
def draw_cumprob_each_duplication(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        for file_name in file_list:
            ...
            X_labels.append("SW-ARQ{}-{}".format(datas[0], datas[1]))
            Y.append(datas[2])
...
def draw_median_by_distance_each_duplication(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        plt.gca().yaxis.set_major_formatter(ticker.ScalarFormatter(useMathText=True))
        plt.gca().ticklabel_format(style="sci",  axis="y",scilimits=(0,0))
        # plt.locator_params(axis='x',nbins=4, min_n_ticks=4)
        plt.xticks([30, 50, 70, 90])
        plt.locator_params(axis='y',min_n_ticks=10)
...
def draw_median_by_duplication_each_distance(data_dict, classify_dict):
    dir_path = "./compare_median_by_duplication_each_distance/"
    check_directory(dir_path)
    file_list_by_conditions = classify_dict["t"]
    ...
    for file_list in file_list_by_conditions:
        ...
        plt.ylabel('Median RTT (s)')
        plt.xticks(X, [str(x) for x in X])
        plt.locator_params(axis='y',min_n_ticks=6)
        plt.grid(True)
...
def draw_collision_by_duplication_each_distance(data_dict, classify_dict):
    dir_path = "./compare_collision_by_duplication_each_distance/"
    check_directory(dir_path)
    file_list_by_conditions = classify_dict["t"]
    ...
    for file_list in file_list_by_conditions:
        ...
        for file_name in file_list:
            data = data_dict[file_name]
            for i in range(len(X_labels)):
                if "d={}".format(data.r) in X_labels[i]:
                    Y[i].append(data.input_data.coll_avg)
                    break
...
def draw_failure_by_duplication_each_distance(data_dict, classify_dict):
    dir_path = "./compare_failure_by_duplication_each_distance/"
    check_directory(dir_path)
    file_list_by_conditions = classify_dict["t"]
    ...
    for file_list in file_list_by_conditions:
        ...
        for file_name in file_list:
            ...
            for i in range(len(X_labels)):
                if "d={}".format(data.r) in X_labels[i]:
                    Y[i].append(data.output_data.transmission_failure_rate)
                    break
        ...
        plt.xlabel('Duplication level (n)')
        plt.ylabel('Transmission failure rate')
        plt.xticks(X, [str(x) for x in X])
        plt.grid(True)
        plt.legend(loc='upper right')
        # plt.locator_params(axis='y',min_n_ticks=8)
...
def draw_energy_by_duplication_each_distance(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        for file_name in file_list:
            data = data_dict[file_name]
            if not data.info_arq in X:
                X.append(data.info_arq)
                y_num = (data.input_data.info_sum_num + data.input_data.ack_sum_num) / len(data.input_data.steps)
                # mol = y_num / NA
                if y_num == 2:
                    y = 0
                else:
...
def draw_regression_median_by_distance_each_rto(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        draw_regression_graph(XY, X_labels, 'Tx-Rx distance (um)', 'Median RTT (s)', fig_name)
...
def draw_regression_mean_by_distance_each_rto(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        draw_regression_graph(XY, X_labels, 'Tx-Rx distance (um)', 'Mean RTT (s)', fig_name)
...
def draw_mean_by_rto_each_distance(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        plt.legend(loc='upper left')
        plt.xticks(x, X)
        plt.locator_params(axis='y',min_n_ticks=5)
...
def draw_regression_jitter_by_distance_each_rto(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        draw_regression_graph(XY, X_labels, 'Tx-Rx distance (um)', 'Jitter of RTT', fig_name)
...
def draw_median_by_retransmission_wait_time_each_distance(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        for file_name in file_list:
            ...
            for i in range(len(X_labels)):
                if "d={}".format(datas[1]) == X_labels[i]:
                    # X[i].append(datas[0])
                    # Y[i].append(datas[2])
                    XY[i][datas[0]] = datas[2]
...
def draw_regression_median_by_retransmission_wait_time_each_distance(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        draw_regression_graph(XY, X_labels, 'Diffusion Coefficient', 'Median RTT (s)', fig_name)
...
def draw_regression_mean_by_retransmission_wait_time_each_distance(data_dict, classify_dict):
    ...
    for file_list in file_list_by_conditions:
        ...
        draw_regression_graph(XY, X_labels, 'Diffusion Coefficient', 'Mean RTT (s)', fig_name)
...
def draw_failure_by_rto_each_distance(data_dict, classify_dict):
    dir_path = "./compare_failure_by_rto_each_distance/"
    check_directory(dir_path)
    file_list_by_conditions = classify_dict["dt"]
    ...
    for file_list in file_list_by_conditions:
        ...
        for file_name in file_list:
            data = data_dict[file_name]
            for i in range(len(X_labels)):
                if "d={}".format(data.r) == X_labels[i]:
                    XY[i][data.retransmitWaitTime] = data.output_data.transmission_failure_rate
                    break
...
def draw_regression_mean_by_distance_each_decomposing(data_dict, classify_dict):
    dir_path = "./regression_mean_by_distance_each_decomposing/"
    check_directory(dir_path)
    file_list_by_conditions = classify_dict["dt"]
    ...
    for file_list in file_list_by_conditions:
        ...
        _, X_labels = search_by_distance_each_decomposing(data_dict, file_list)
        XY = [{} for i in range(len(X_labels))]
...
def draw_regression_median_by_distance_each_decomposing(data_dict, classify_dict):
    dir_path = "./regression_median_by_distance_each_decomposing/"
    check_directory(dir_path)
    file_list_by_conditions = classify_dict["dt"]
...
def draw_regression_jitter_by_distance_each_decomposing(data_dict, classify_dict):
    dir_path = "./regression_jitter_by_distance_each_decomposing/"
    check_directory(dir_path)
    file_list_by_conditions = classify_dict["dt"]
...
def draw_graph(data_dict, classify_dict):
    ...
    # draw_median_by_duplication_each_distance(data_dict, classify_dict)
    ...
    # draw_collision_by_duplication_each_distance(data_dict, classify_dict)
    # draw_failure_by_duplication_each_distance(data_dict, classify_dict)
    # draw_energy_by_duplication_each_distance(data_dict, classify_dict)
    ...
    # draw_failure_by_rto_each_distance(data_dict, classify_dict)
