...
class AllData:
    ...
    def define_file_name(self):
        ...
        self.file_name = "TxRx{}_ARQ{}-{}_{}-{}_{}".format(self.r, self.info_arq, self.ack_arq, self.info_type.name, self.ack_type.name, str(self.step_length).replace('.', '-'))
    ...
    def get_mean_by_distance(self):
        ret_arr = [self.r, self.output_data.mean, self.output_data.analytical_model.rtt]
        if self.is_ptime:
            ret_arr.append(self.output_data.txrx_mean)
        return ret_arr
    ...
    def get_median_by_distance_each_duplication(self):
        return [self.r, self.info_arq, self.ack_arq, self.output_data.med, self.output_data.analytical_model.rtt]

    def get_mean_by_distance_each_duplication(self):
        return [self.r, self.info_arq, self.ack_arq, self.output_data.mean, self.output_data.analytical_model.rtt]

    def get_mean_by_duplication_each_distance(self):
        return [self.r, self.info_arq, self.output_data.mean]

    def get_txrx_mean_by_distance_each_duplication(self):
        return [self.r, self.info_arq, self.ack_arq, self.output_data.txrx_mean, self.output_data.analytical_model.rtt]

    def get_std_by_duplication_each_distance(self):
        return [self.r, self.info_arq, self.output_data.std]

    def get_median_by_duplication_each_distance(self):
        return [self.r, self.info_arq, self.output_data.med]

    def get_median_by_diffusioncoefficient(self):
        return [self.config.config_dict["stepLengthX"], self.output_data.med]

    def get_csv_data(self):
        return [self.r, self.info_arq, self.ack_arq, self.output_data.mean, self.output_data.med, self.output_data.txrx_mean]
